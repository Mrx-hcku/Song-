...
<body>
    ...
    <div class="main-container">
        ...
        <div id="favorites-section" class="content-section">
            <h2 class="heading-module" style="border-color: var(--primary-accent);">My Favorites</h2>
            <div id="favoritesList" class="track-list">
                <p class="loading-state">You haven't added any songs to favorites yet.</p>
            </div>
            <div style="text-align:center; padding: 20px;">
                <button onclick="clearFavorites()" style="background:var(--secondary-accent); color:var(--text-high); border:none; padding: 10px 20px; border-radius: 50px; cursor:pointer; font-weight:600;">Clear All Favorites</button>
            </div>
        </div>
        </div>
    ...
    <div class="navigation">
        ...
        <div class="nav-item" data-target="favorites-section" onclick="changeSection(this, true)">
    ...
    <script>
        ...
        let isPlayerMinimized = false; // NEW state variable
        let favorites = []; // NEW: Array to hold favorite track objects
        ...
        function clearFavorites() {
             if(confirm("Are you sure you want to clear all your favorite songs?")) {
                 favorites = [];
                 saveFavorites();
                 renderFavorites();
                 // If the queue is playing favorites, stop playback
                 if(document.querySelector('.nav-item.active').getAttribute('data-target') === 'favorites-section') {
                      audioPlayer.pause();
                      currentTrackIndex = -1;
                      updatePlayerUI(null);
                 }
        ...
        function toggleFavorite(track) {
            ...
            if (index > -1) {
                ...
                // Add to favorites
                // Ensure the track object has the minimum required properties (id, name, image, artist)
                favorites.push(track);
            }

            saveFavorites();
            
            // Re-render favorites if that section is visible
        ...
        /** Renders the current favorites list to the favorites section */
        function renderFavorites() {
            renderTrackList(favorites, favoritesListDiv, false, 'favorites');
        }
        ...
        function updateFavoriteButtons(trackId) {
            ...
            document.querySelectorAll(`.track-item[data-id="${trackId}"] .favorite-button`).forEach(button => {
        ...
        function changeSection(navItem, shouldRenderFavorites = false) {
            ...
            // NEW: If switching to favorites, render the list
            if (shouldRenderFavorites || targetId === 'favorites-section') {
                renderFavorites();
            }
        }
        ...
        function renderTrackList(tracks, container, isSearch = false, source = 'queue_or_search') {
            container.innerHTML = ''; // Clear previous results
            container.classList.remove('module-grid');
            container.classList.add('track-list'); // Ensure correct styling
            
            if (!tracks || tracks.length === 0) {
                container.innerHTML = `<p class="loading-state">${isSearch ? 'No songs found for your search.' : (source === 'favorites' ? 'You haven\'t added any songs to favorites yet.' : 'Queue is empty.')}</p>`;
                return;
            }
            
            // NEW: Function to handle track playback from a specific list
            ...
            tracks.forEach((track, index) => {
                ...
                const isPlayingInThisList = (
                    ...
                    currentPlaylist.includes(track) // Basic check to see if the track is in the current playlist
                );
                ...
                trackItem.innerHTML = `
                    ...
                    <button class="favorite-button ${favorites.some(fav => fav.id === track.id) ? 'is-favorite' : ''}" 
                            title="Add to Favorites" 
                            onclick="event.stopPropagation(); toggleFavorite(${JSON.stringify(track).replace(/"/g, '&quot;')})">
                        <svg viewBox="0 0 24 24"><path d="M12,21.35L10.55,20.03C5.4,15.36 2,12.27 2,8.5C2,5.41 4.42,3 7.5,3C9.24,3 10.91,3.81 12,5.08C13.09,3.81 14.76,3 16.5,3C19.58,3 22,5.41 22,8.5C22,12.27 18.6,15.36 13.45,20.03L12,21.35Z" /></svg>
                    </button>
                    `;
        ...
        function renderQueue() {
            renderTrackList(currentPlaylist, queueListDiv, false, 'queue');
        }
        ...
        async function initialLoad() {
            ...
            // Render favorites list on load so it's ready if the user navigates there
            renderFavorites();
        }
